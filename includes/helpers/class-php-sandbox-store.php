...
/**
 * PHP_Sandbox_Store — source-of-truth + sandbox for admin/AI PHP snippets.
 *
 * The canonical record is a private `novamira_php_snippet` CPT whose meta holds
 * the raw code, run context, validation report, and a sha256 integrity hash.
 * An executable file is written to the sandbox ONLY while a snippet is active
 * (uploads/novamira-sandbox/snippets/{id}.php, .htaccess-blocked); a draft has
 * no runnable artifact on disk at all. Post status is the activation flag:
 * `publish` = active, `draft` = inactive.
 *
 * Activation is the human approval gate: create/update only ever produce DRAFTS
 * (the MCP tools cannot reach set_status), and set_status('active') re-runs
 * the validator and refuses anything with a critical finding.
 *
 ...
 * @since 1.0.0
 */
final class PHP_Sandbox_Store {

    const POST_TYPE       = 'novamira_php_snippet';
    const META_CODE       = '_novamira_snippet_code';
    const META_CONTEXT    = '_novamira_snippet_context';
    const META_HOOK       = '_novamira_snippet_hook';
    const META_PRIORITY   = '_novamira_snippet_priority';
    const META_VALIDATION = '_novamira_snippet_validation';
    const META_HASH       = '_novamira_snippet_hash';
    const META_ERROR      = '_novamira_snippet_error';

    /** Shared sandbox base (created by the Widget Builder too). */
    const SANDBOX_DIR = 'novamira-sandbox';

    /** Allowed run contexts. */
    const CONTEXTS = ['shortcode', 'hook', 'both'];
    ...
    public static function register_post_type(): void {
        register_post_type(
            ...
            [
                ...
                'labels'              => ['name' => __('Novamira PHP Snippets', 'novamira-adrianv2')],
            ]
        );
    }
     ...
     * code: manage_options AND unfiltered_html.
     */
    ...
    public static function func_name(int $post_id): string {
        return 'novamira_php_snippet_' . $post_id;
    }
    ...
    public static function ensure_sandbox() {
        ...
        if (!wp_mkdir_p($dir)) {
            return new \WP_Error('sandbox_unwritable', __('Could not create the snippet sandbox directory under uploads. Check filesystem permissions.', 'novamira-adrianv2'));
        }
        self::write_file(self::sandbox_dir() . '/index.php', "<?php\n// Silence is golden.\n");
        self::write_file($dir . '/index.php', "<?php\n// Silence is golden.\n");
        self::write_file(
            self::sandbox_dir() . '/.htaccess',
            "# Generated by Novamira AdrianV2 - block direct PHP access.\n<FilesMatch \"\\.php$\">\n<IfModule mod_authz_core.c>\nRequire all denied\n</IfModule>\n<IfModule !mod_authz_core.c>\ndeny from all\n</IfModule>\n</FilesMatch>\n"
        );
        return true;
    }
    ...
    public static function create_draft(array $args) {
        if (!self::can_edit()) {
            return new \WP_Error('forbidden', __('You do not have permission to create PHP snippets (requires manage_options and unfiltered_html).', 'novamira-adrianv2'));
        }

        $code       = isset($args['code']) ? (string) $args['code'] : '';
        $validation = PHP_Sandbox_Validator::validate($code);
        if (!$validation['valid']) {
            return new \WP_Error('invalid_php', sprintf(/* translators: %s: parse error */ __('The snippet is not valid PHP: %s', 'novamira-adrianv2'), $validation['parse_error']), ['validation' => $validation]);
        }
        if (!$validation['safe']) {
            return new \WP_Error('unsafe_php', __('The snippet was blocked by the security validator (critical finding). See the validation report.', 'novamira-adrianv2'), ['validation' => $validation]);
        }

        $title = isset($args['title']) && '' !== trim((string) $args['title'])
            ? sanitize_text_field((string) $args['title'])
            : __('PHP Snippet', 'novamira-adrianv2');
    ...
    public static function update(int $post_id, array $args) {
        if (!self::can_edit()) {
            return new \WP_Error('forbidden', __('You do not have permission to update PHP snippets.', 'novamira-adrianv2'));
        }
        $post = get_post($post_id);
        if (!$post || self::POST_TYPE !== $post->post_type) {
            return new \WP_Error('not_found', __('Snippet not found.', 'novamira-adrianv2'));
        }
        ...
        if (isset($args['title']) && '' !== trim((string) $args['title'])) {
            wp_update_post(['ID' => $post_id, 'post_title' => sanitize_text_field((string) $args['title'])]);
        }
    ...
    private static function write_executable(int $post_id, string $code) {
        ...
        $body = PHP_Sandbox_Validator::strip_tags($code);
        $func = self::func_name($post_id);
        $php  = "<?php\n"
            . "// Novamira PHP Snippet {$post_id} - generated from the snippet stored in the database.\n"
        ...
        if (!self::write_file($path, $php)) {
            return new \WP_Error('write_failed', __('Could not write the snippet file to the sandbox.', 'novamira-adrianv2'));
        }
    ...
    /**
     * Activates or deactivates a snippet. Activation re-validates and writes
     * the executable file; deactivation removes it. This is the human approval
     * gate - never exposed to the MCP tools.
     *
     * @param int    $post_id Snippet ID.
    ...
    public static function set_status(int $post_id, string $status) {
        if (!self::can_edit()) {
            return new \WP_Error('forbidden', __('You do not have permission to change snippet status.', 'novamira-adrianv2'));
        }
        $post = get_post($post_id);
        if (!$post || self::POST_TYPE !== $post->post_type) {
            return new \WP_Error('not_found', __('Snippet not found.', 'novamira-adrianv2'));
        ...
        if ('active' === $status) {
            ...
            if (!$validation['valid'] || !$validation['safe']) {
                update_post_meta($post_id, self::META_VALIDATION, wp_slash((string) wp_json_encode($validation)));
                return new \WP_Error('activation_blocked', __('Cannot activate - the snippet fails validation. Fix the flagged code and try again.', 'novamira-adrianv2'), ['validation' => $validation]);
            }
            $written = self::write_executable($post_id, $code);
            if (is_wp_error($written)) {
                return $written;
    ...
    public static function delete(int $post_id) {
        if (!self::can_edit()) {
            return new \WP_Error('forbidden', __('You do not have permission to delete PHP snippets.', 'novamira-adrianv2'));
        }
        $post = get_post($post_id);
        if (!$post || self::POST_TYPE !== $post->post_type) {
            return new \WP_Error('not_found', __('Snippet not found.', 'novamira-adrianv2'));
        ...
        self::rebuild_manifest();
        return ['success' => true, 'snippet_id' => $post_id];
    }
    ...
    public static function summary(int $post_id) {
        ...
        return [
            ...
            'shortcode'  => '[novamira_snippet id="' . (int) $post_id . '"]',
    ...
    public static function get_validation(int $post_id): array {
        ...
        if (empty($raw)) {
            return ['valid' => true, 'safe' => true, 'parse_error' => '', 'findings' => []];
        }
        $data = json_decode((string) $raw, true);
        return is_array($data) ? $data : ['valid' => true, 'safe' => true, 'parse_error' => '', 'findings' => []];
    }
