...
/**
 * V4_Content_Extractor — normalized content extraction from an Elementor element tree.
 *
 ...
 * @since 1.0.0
 */
final class V4_Content_Extractor {
        ...
        switch ($type) {
            case 'heading':
                ...
                if ('' !== $text) {
                    ...
                    if (null !== $level) {
                        ...
                        $out['text_blocks'][] = ['text' => $plain, 'element_id' => $id];
                    }
                }
                self::collect_text_color($id, $settings, ['title_color'], $ancestor_bg, $out);
                self::maybe_link(self::scalar(self::deep($settings, ['link', 'url'])), $text, $site_host, $id, $out);
                break;
            ...
            case 'e-heading':
                ...
                if ('' !== $atext && null !== $alevel) {
                    ...
                    $out['headings'][]       = ['level' => $alevel, 'text' => $aplain, 'element_id' => $id];
            ...
            case 'e-paragraph':
                ...
                if ('' !== $html) {
                    $out['text_blocks'][] = ['text' => self::plain_text($html), 'element_id' => $id];
                    self::parse_markup($html, $site_host, $id, $out);
                }
                self::collect_text_color($id, $settings, ['text_color', 'color'], $ancestor_bg, $out);
                break;
            ...
            case 'e-button':
                ...
                if ('' !== $btxt) {
                    $out['text_blocks'][] = ['text' => $btxt, 'element_id' => $id];
                }
                self::maybe_link(self::scalar(self::deep($settings, ['link', 'url'])), $btxt, $site_host, $id, $out);
                break;
            ...
            case 'image-box':
                ...
                if ('image-box' === $type) {
                    ...
                    if ('' !== $title) {
                        ...
                        $out['headings'][] = ['level' => $lvl ?? 3, 'text' => $title, 'element_id' => $id];
                    }
                    if ('' !== $desc) {
                        $out['text_blocks'][] = ['text' => $desc, 'element_id' => $id];
                    }
                }
                break;
            ...
            case 'icon-list':
                ...
                if (is_array($items)) {
                    foreach ($items as $item) {
                        ...
                        if ('' !== $itxt) {
                            $out['text_blocks'][] = ['text' => $itxt, 'element_id' => $id];
                        }
                        self::maybe_link(self::scalar(self::deep($item, ['link', 'url'])), $itxt, $site_host, $id, $out);
                    }
                }
                break;
            ...
            case 'html':
                ...
                if ('' !== $raw) {
                    $out['text_blocks'][] = ['text' => self::plain_text($raw), 'element_id' => $id];
    ...
    private static function collect_image(string $id, array $settings, array &$out): void {
        ...
        if (isset($image['$$type'])) {
            $image = is_array($image['value'] ?? null) ? $image['value'] : [];
        }
        if (!is_array($image)) {
            return;
        }
    ...
    private static function resolve_bg(array $settings): ?string {
        foreach (['background_color', '_background_color'] as $key) {
            $val = self::scalar($settings[$key] ?? '');
            if ('' !== $val && '#' === substr($val, 0, 1)) {
                return $val;
            }
    ...
    private static function parse_markup(string $html, string $site_host, string $id, array &$out): void {
        if (preg_match_all('/<h([1-6])\b[^>]*>(.*?)<\/h\1>/is', $html, $hm, PREG_SET_ORDER)) {
            foreach ($hm as $m) {
                ...
                if ('' !== $txt) {
                    $out['headings'][] = ['level' => (int) $m[1], 'text' => $txt, 'element_id' => $id];
                }
            }
        }
	...
	private static function scalar($value): string {
		if (is_array($value)) {
			if (array_key_exists('value', $value) && array_key_exists('$$type', $value)) {
                $unwrapped = V4_Props::unwrap($value);
                return is_scalar($unwrapped) ? (string) $unwrapped : '';
			}
			return '';
		}
