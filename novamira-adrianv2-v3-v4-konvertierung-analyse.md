...
2. neue Ability `novamira-adrianv2/convert-page-v3-to-v4` bauen,
...
Die Docs/Skills erwarten teils andere Felder wie `atomic_supported`, `global_classes_available` oder page-level Detection. Fuer die Migration reicht die aktuelle Ability nicht aus.

Empfehlung:

`detect-elementor-version` rueckwaertskompatibel erweitern:
...
Der externe Plan nennt `convert-page-v3-to-v4` als fehlend. Das stimmt weiterhin.

Suchergebnis:

- keine Ability,
- keine Klasse,
- keine rekursive Converter-Funktion,
...
`class-batch-create-variables.php` prueft:

```php
class_exists('Novamira\\AdrianV2\\Helpers')
```

Diese Klasse existiert nicht. Dadurch kann die Ability faelschlich `helpers_not_loaded` liefern.
...
- Converter nutzt `Elementor_Document_Saver::save_data()`.
...
```text
includes/helpers/class-v3-to-v4-converter.php
```

Empfohlene Verantwortlichkeiten:

- Ability-Klasse: Input, Permissions, Rollback, Save, Response.
- Converter-Helper: rekursive Tree-Konvertierung.
- Style-Mapper: V3 Settings zu V4 props/styles/classes.
- Widget-Mapper: V3 Widget Types zu Atomic Widgets.
...
- Helper Namespace Imports korrigieren.
...
- neue Ability `convert-page-v3-to-v4`,
- Dry Run,
- Basis-Mapping fuer Section/Column/Container,
- Basis-Widgets: heading, text, button, image, divider, spacer,
- Fallback fuer unknown Widgets,
- Speichern ueber `Elementor_Document_Saver`.

### P2: Design-System-Mapping

- `kit-convert-v3-to-v4` integrieren,
- Variables wiederverwenden,
- Global Classes zuweisen,
- lokale Styles reduzieren,
- responsive Styles mappen.
